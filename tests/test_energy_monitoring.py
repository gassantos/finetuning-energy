...
class TestRobustGPUMonitorEnhanced:
    ...
    def test_baseline_establishment_simulation(self):
        ...
        # Simular estratégia de coleta de dados
        mock_strategy = Mock()
        mock_strategy.collect_data.return_value = [{"gpu_id": 0, "power_w": 50, "name": "GPU0"}]
        
        monitor._gpu_strategy = mock_strategy
...
class TestIntegrationScenarios:
    ...
    def test_complete_monitoring_cycle_simulation(self):
        ...
        # Simular estratégia de coleta de dados
        mock_strategy = Mock()
        mock_strategy.collect_data.return_value = [{"gpu_id": 0, "power_w": 150, "temperature_c": 70, 
                "utilization_percent": 80, "memory_used_mb": 8000, "name": "TestGPU"}]
        ...
        monitor.energy_data = [
            {
                ...
                "method": "nvitop",
                "gpus": [{"gpu_id": 0, "power_w": 150 + i * 5, "temperature_c": 70 + i,
                         "utilization_percent": 80, "memory_used_mb": 8000, "name": "TestGPU"}]
            }
            for i in range(10)
        ]
